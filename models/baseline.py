...
# remove lowest 10% of market cap
# df = df[df['ME'] > df['ME'].quantile(0.01)]
...
while start_date in df_by_date:
    ...
    # test -= 1
    # color = '\033[92m' if test > 0 else '\033[91m'
    # reset = '\033[0m'

    # tests.append(test*100)

    # print(f"{start_date} {end_date} {color}{round( test * 100, 2)}%{reset}")
